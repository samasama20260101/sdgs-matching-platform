...
3. 新規メンバー追加、停止、復帰を操作できる。
4. 「登録済みアカウントを所属に追加」と「所属解除」の導線が表示されない。
5. APIへ直接 `registration_type = existing` または `status = LEFT` を送っても拒否される。
6. SOS案件へ申し出できる。
7. SOS側で承認できる。
8. チャットに団体名と担当者名が表示される。
9. 内部メモの `自団体のみ` と `担当サポーター間` を使い分けられる。
10. SOS側に内部メモが表示されない。
11. サポーターOWNERで団体情報・団体所在地・活動地域を保存できる。
12. SOSプロフィールで非公開情報・表示名・地域・詳細住所を保存できる。
13. 確認完了後、Vercel Production環境変数で `MAINTENANCE_MODE=false` または未設定に戻す。
14. Vercel Productionを再デプロイする。
15. 通常ブラウザでProduction URLを開き、`/maintenance` に誘導されないことを確認する。
