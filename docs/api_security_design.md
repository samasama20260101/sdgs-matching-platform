...
DB負荷・Disk IOの運用方針は `docs/supabase_io_and_db_access_guidelines.md` も確認します。
...
| `src/lib/api/validation.ts` | API入力値のUUID検証 |
...
- 画面更新は原則60秒以上のポーリングで対応する
- 非表示タブではポーリングを停止する
- タブ再表示時に1回だけ再取得する
- 管理画面一覧は自動ポーリングせず、手動更新や検索を優先する
...
- URL params、query string、request bodyのUUIDは `isUuid()` / `normalizeUuidList()` で検証しているか
- UUIDカラムに空文字 `''` を渡していないか
- 定期更新が必要な画面では、60秒以上・非表示タブ停止になっているか
...
| メッセージ送信後 | 手動更新または最大60秒程度で相手画面へ反映される |
| 不正なUUID / 空文字IDでAPIを叩く | DBエラーではなく `400` または `404` |
