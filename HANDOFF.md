...
残るのは未コミット作業の扱い(次の一手1)と、以前からの継続タスクのみ。
...
## 次の一手
1. 未コミット2件の扱いを決める: `src/app/api/cron/auto-close-cases/route.ts`(MATCHED無活動の自動CLOSEDを14→30日)はコミットするか破棄するか。`docs/sdgs_impact_dashboard_design.md`(§5改訂・§10)はコミット
