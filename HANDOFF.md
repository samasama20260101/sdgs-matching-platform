...
- **個人情報マスク検証ラボを新設(2026-09-01・devのみ)**: `/admin/mask-lab`(ADMIN専用・リンクなし)。目的は「AIに渡す直前のデータ最小化層」の実力検証で、依頼者が目で見て判断するための実験台。層1=正規表現、層2=+kuromoji.js固有表現(辞書は初回CDN取得)。入力文は送信・保存ゼロ。壁打ちでの決定: 適用先はAI直前のみ(サポーター表示はマスクしない)・消しすぎ許容・完璧は求めない(無学習契約との二段構え)。既知の限界: ひらがな建物名は接頭辞までしか獲れない。次: 依頼者がStagingで触って層2の要否と実装方針を判断

- **関数リージョンの東京化(`8a9f5d8`)は本番未反映**。vercel.json に `"regions":["hnd1"]` を追加(従来は既定のiad1=米国東海岸で、東京のSupabaseと毎回太平洋往復だった)。Staging で hnd1 実行を確認済み。次の main マージで本番に乗る
