...
## 現在地
**2026-09-03 PR #31 で東京リージョン化+マスク検証ラボを本番反映(main = `93099a7`)。本番の関数実行が hnd1(東京)になったことを実測確認済み。dev と本番はコード的に同一。**
前回まで: PR #29(2026-08-29、実操作検収済み)、PR #30(auto-close 30日化)。DB操作はいずれもなし。
...
- **個人情報マスク検証ラボ(2026-09-01新設、2026-09-03 PR #31で本番反映済み)**: `/admin/mask-lab`(ADMIN専用・リンクなし)。辞書はページを開くと自動でバックグラウンド読み込み。目的は「AIに渡す直前のデータ最小化層」の実力検証で、依頼者が目で見て判断するための実験台。層1=正規表現、層2=+kuromoji.js固有表現(辞書は初回CDN取得)。入力文は送信・保存ゼロ。壁打ちでの決定: 適用先はAI直前のみ(サポーター表示はマスクしない)・消しすぎ許容・完璧は求めない(無学習契約との二段構え)。既知の限界: ひらがな建物名は接頭辞までしか獲れない。次: 依頼者がStagingで触って層2の要否と実装方針を判断
