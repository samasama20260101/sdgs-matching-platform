...
## 現在地
dev には 1対1化・種別「その他」・AI分析修正・共有メモ削除がマージ済み(PR #26)。
取消まわりの改善は PR #27 としてレビュー/Staging確認待ち。
**本番(main)は6/12から通常フローのAI分析が壊れたまま。dev→main の反映が最優先。**
...
## 次の一手
1. Staging で PR #27 の確認: 通常案件でマッチ→SOS取消→モーダル文言/サポーター側チャットのシステムメッセージ/「✕取消済み」チップを確認 → 問題なければ #27 をdevへマージ
2. **dev→main のPRを作成し本番反映**(通常フローAI分析の復旧。本番は現在も登録時に「AI分析に失敗しました」)
3. 本番反映後: 既存サポーターの種別変更 `UPDATE organizations SET supporter_type = 'OTHER' WHERE name = '対象団体名';` を本番SQL Editorで(ユーザー自身が実行)
4. 拡散タスク(未着手): Resend Proアップグレード / Instagramリンク設定+投稿 / pptxスライド12とリーフレットの連絡先記入 / 八代市「郡築」表記確認
5. 技術負債: スキーマ差分照合スクリプト(read-only本番PG vs Staging)をリリース手順に組込み
