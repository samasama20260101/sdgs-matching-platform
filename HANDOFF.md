...
## 現在地
**本番反映すべて完了(2026-08-16)。** コード: PR #26/#27/#28マージ・デプロイ済み(main=dev)。
DB: display_id形式統一・UNIQUE付与・ケアラーズカフェ モンステラのOTHER化(organizations/users両方)をStaging・本番で実行・検証済み。
本番スモークテストもユーザー確認済み。残タスクは拡散タスクと技術負債のみ。
...
## 次の一手
1. 拡散タスク(未着手・ユーザー作業): Resend Proアップグレード / Instagramリンク設定+投稿 / pptxスライド12とリーフレットの連絡先記入 / 八代市「郡築」表記確認
2. 技術負債: スキーマ差分照合スクリプト(read-only本番PG vs Staging)をリリース手順に組込み
3. 技術負債: admin/stats API がレガシー列 users.supporter_type を読んでいる → organizations 参照に変更(種別変更のたびに2テーブル更新が必要になっている原因。ケアラーズカフェ モンステラのOTHER化では両方手動更新で対応済み 2026-08-16)
