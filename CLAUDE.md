...
ファイルの**最初の行**（コメントより後、importより前）に記述すること。

```typescript
// src/app/xxx/page.tsx   ← コメントはOK
'use client';             ← ここ
import { useState } ...   ← importはこの後
```
...
| フォーム | フィールド | 上限 |
|---|---|---|
| 新規登録 | メールアドレス | 254文字（RFC 5321） |
| 新規登録 | パスワード | 64文字 |
| 新規登録 | お名前・表示名 | 64文字 |
| 新規登録 | 電話番号 | 15桁（ハイフン除去してDB保存） |
| 相談登録 | Q1〜Q5 その他 | 200文字 |
| 相談登録 | いま何が起きているか | 1000文字 |
| 相談登録 | いつから・どうなりたいか | 200文字 |
| お問い合わせ | 詳細 | 1000文字 |
| 住所 | 都道府県/Province | 50文字（インドネシア最長州名25文字対応） |
| 住所 | 市区町村/City | 50文字 |
| 住所 | 番地・建物名 | 100文字 |
| サポーター作成 | 組織名・担当者名・表示名 | 64文字 |
| サポーター作成 | メール | 254文字 |
| サポーター作成 | 電話番号 | 15桁 |
| プロフィール | 自己紹介文 | 500文字 |
| プロフィール | SNS外部リンク全フィールド | 500文字 |

---

## DBスキーマ補足（Stagingで確認済み）
...
| `src/app/api/admin/users/[id]/route.ts` | ユーザー停止・削除API |
